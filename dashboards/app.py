...
# Carregando os dados
df = pd.read_csv("6/dados/dataset.csv")
...
def group_by(df, col):
    ...
    # Agregação
    grouped = df.groupby(by=col, as_index=False).agg(
        {"Venda": "sum", "Lucro": "sum", "Quantidade": "sum", "Desconto": "mean"}
    )

    # Calculando a margem de lucro
    grouped["Margem_Lucro"] = np.multiply(
        np.divide(grouped["Lucro"], grouped["Venda"]), 100
    ).round(2)
...
figura_1 = (
    px.box(
        ...
        labels={
            ...
            "Margem_Lucro": "Margem de Lucro Bruto",
        },
    )
...
figura_1.add_vline(x=0.41, line_width=1, line_dash="dash", line_color="red")

# Sunburst Plot
figura_2 = px.sunburst(
    data_frame=df,
...
figura_2.update_layout(
    title={
        "text": "Quantidade Vendida e Lucro Para Cada Tipo de Produto",
        "font": title_font,
        "x": 0.5,
        "y": 0.02,
        "xanchor": "center",
...
def agrupa_estados(dataframe):
    ...
    estados = dataframe.groupby(
        ["Estado", "Codigo_Estado", "Regiao"], as_index=False
    ).agg({"Venda": "sum", "Lucro": "sum", "Desconto": "mean", "Quantidade": "sum"})
    # Calculando a margem de lucro relativo
    estados["Margem_Lucro"] = np.multiply(
        np.divide(estados["Lucro"], estados["Venda"]), 100
    ).round(2)
...
us_map.update_layout(
    title={
        "text": "Margem de Lucro Bruto - Mapa USA",
        "font": title_font,
        "x": 0.5,
        "y": 0.9,
        "xanchor": "center",
...
sidebar = html.Div(
    [
        ...
        html.P("Versão 1.0", className="fixed-bottom text-white p-2"),
    ],
    className="bg-dark",
    style={
        "position": "fixed",
        ...
        dbc.Row(
            [
                dbc.Col(
                    [
                        ...
                        html.P(
                            "R$ {}".format(
                                str(
                                    locale.format(
                                        "%.4f", df.Venda.sum().round(2), grouping=True
                                    )
                                )
                            ),
                        ...
                        html.P(
                            "R$ {}".format(
                                str(
                                    locale.format(
                                        "%.4f", df.Lucro.sum().round(2), grouping=True
                                    )
                                )
                            ),
                    ...
                    width=2,
                    style={"border": "2px solid black", "borderRight": False},
                ),
                dbc.Col(
                ...
                dbc.Col(
                    ...
                    width={"size": 5, "offset": 0},
                    style={"border": "2px solid black", "borderLeft": False},
                ),
            ],
            no_gutters=True,
            justify="around",
            ...
            [
                dbc.ListGroup(
                    [
                        ...
                        dbc.ListGroupItem(
                            "2) Média de Vendas de R$ 230.14 por transação."
                        ),
                        dbc.ListGroupItem(
                            "3) Média de Lucro de R$ 28.69 por transação"
                        ),
                    ],
                ),
            ],
...
card_con = dbc.Card(
    [
        dbc.CardHeader(
            html.H4("Problemas Detectados", className="card-title"),
            className="bg-primary text-white",
        ),
        dbc.CardBody(
            [
                dbc.ListGroup(
                    [
                        dbc.ListGroupItem(
                            "1) A margem de lucro bruto parece estar diminuindo com o aumento do desconto nos produtos."
                        ),
                        dbc.ListGroupItem(
                            "2) Além de 40% de desconto, a loja sofreu apenas perdas."
                        ),
                        dbc.ListGroupItem(
                            "3) As quantidades vendidas não estão aumentando com o desconto maior."
                        ),
                    ],
                ),
            ],
...
card_final = dbc.Card(
    [
        dbc.CardHeader(
            html.H4("Conclusão", className="card-title"),
            className="bg-primary text-white",
        ),
        dbc.CardBody(
            [
                html.P(
                    [
                        "A loja tem um bom desempenho quando nenhum desconto ou desconto inferior a 20% é aplicado.",
                        html.Hr(),
                        "A loja pode se beneficiar reduzindo o desconto em itens de produtos deficitários.",
                        html.Hr(),
...
def update_output(option):
    ...
    if option == "Transactions":
        ...
        fig.add_trace(
            go.Histogram(x=dff["Tipo Entrega"], name="Tipo Entrega"), row=1, col=1
        )
        ...
        fig.add_trace(
            go.Histogram(x=dff["Categoria"], name="Categoria de Produto"), row=2, col=2
        )
        ...
        fig.add_trace(
            go.Bar(x=ship["Tipo Entrega"], y=ship[option], name="Tipo Entrega"),
            row=1,
            col=1,
        )
        ...
        fig.add_trace(
            go.Bar(x=seg["Segmento"], y=seg[option], name="Segmento"), row=1, col=2
        )

        reg = group_by(dff, "Regiao")

        fig.add_trace(
            go.Bar(x=reg["Regiao"], y=reg[option], name="Região"), row=2, col=1
        )

        cat = group_by(dff, "Categoria")

        fig.add_trace(
            go.Bar(x=cat["Categoria"], y=cat[option], name="Categoria de Produto"),
            row=2,
            col=2,
        )
    ...
    figura_3 = (
        px.bar(
            ...
            color_continuous_midpoint=0,
            title=("{} Através dos Estados Americanos".format(option)),
        )
        .update_traces(marker_line_color="rgb(8,48,107)", marker_line_width=1)
        .update_layout(
            title={
                "font": title_font,
                "x": 0.5,
...
def update_output(tab):
    dff = df.copy()
    dff["Margem_Lucro"] = np.multiply(np.divide(dff["Lucro"], dff["Venda"]), 100).round(
        2
    )
    dff.round(2)
